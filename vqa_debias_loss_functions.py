...
class Plain(DebiasLossFn):
    def forward(self, hidden, logits, bias, labels,type_b):
...
class NewLoss(DebiasLossFn):
    ...
    def forward(self, hidden, logits, bias, labels, type_b):
        ...
        weights = (1 - 2 * type_b) ** 5  # 2-57.56   0.5-57.39
        ...
        loss = -(log_prob * labels + (1 - labels) * log_one_minus_prob)
        loss *= weights
        loss = loss.sum()/(weights/2274).sum()
...
class Plain_type(DebiasLossFn):
    def forward(self, hidden, logits, bias, labels, type_b):
        weights = (1 - 2 * type_b) ** 5
