...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["learned_mixin", "reweight", "bias_product", "none", 'new_loss_10', 'new_loss','plain_type'],
    ...
    parser.add_argument('--p_norm', type=float, default=0.8)
...
def pnorm(weights, p):
    ...
    # avg = torch.mean(normB)
    
    ws = weights.clone()
    for i in range(weights.size(0)):
        # abs_r = normB[i] - avg
        # ws[i] = torch.abs(abs_r) / torch.pow(normB[i], p) * ws[i]
...
def evaluate(model,dataloader,qid2type):
    ...
    for v, q, a, b,qids,hintscore, type_b in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
...
def main():
    ...
    model_dict = torch.load(args.model_state)
    print(model_dict['classifier.main.3.weight_v'])
    model_dict['classifier.main.3.weight_v'] = pnorm(model_dict['classifier.main.3.weight_v'], args.p_norm)
    #model_dict['classifier.main.3.weight_v'] = model_dict['classifier.main.3.weight_v']/70
    print(model_dict['classifier.main.3.weight_v'])
