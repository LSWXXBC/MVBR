...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["learned_mixin", "reweight", "bias_product", "none",'focal','new_loss','new_loss_X','plain_type','ct', 'ct_10'],
        help="Kind of ensemble loss to use")
    parser.add_argument(
...
def type_bias(train_dset,eval_dset):
    # Compute the bias:
    ...
    question_type_to_bias = {}
    x = []

    for q_type, count in question_type_to_count.items():
        bias_array = np.ones(answer_voc_size, np.float32)

        bias_score = float(count)/float(m)
        x.append(bias_score)
        bias_array = bias_score*bias_array

        question_type_to_bias[q_type] = bias_array
    ...
    for ds in [train_dset,eval_dset]:
        for ex in ds.entries:
            ...
            ex["type_b"] = question_type_to_bias[q_type]
...
def main():
    ...
    train_dset = VQAFeatureDataset('train', dictionary, dataset=dataset,
                                   cache_image_features=args.cache_features, ratio=1)
    ...
    type_bias(train_dset, eval_dset)
    ...
    elif args.debias == "plain_type":
        model.debias_loss_fn = Plain_type()
    ...
    elif args.debias=='new_loss':
        model.debias_loss_fn = NewLoss(args.entropy_penalty)
    elif args.debias=='new_loss_X':
        model.debias_loss_fn = NewLoss_X(args.entropy_penalty)
    elif args.debias=='ct':
        model.debias_loss_fn = ContentLoss(args.entropy_penalty)
    elif args.debias=='ct_10':
        model.debias_loss_fn = ContentLoss_10(args.entropy_penalty)
    else:
