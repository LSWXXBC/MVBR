...
from class_re_sample import class_re_sample_forward, test_fun, num_typ, huatu
import random
...
def _load_dataset(dataroot, name, img_id2val, dataset, ratio=1.0):
    """Load entries
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='data', dataset='cpv2',
                 use_hdf5=False, cache_image_features=False, ratio=1):
        ...
        # print("num_ans_candidates length:", self.num_ans_candidates)
        ...
        self.entries = _load_dataset(dataroot, name, imgid2idx, dataset=dataset, ratio=self.ratio)
    ...
    def __getitem__(self, index):
        ...
        if self.name=='train':
            ...
            if "bias" in entry:
                return features, ques, target, entry["bias"], train_hint,type_mask,notype_mask,ques_mask, entry['type_b']

            else:
                return features, ques,target, 0, train_hint,0
        else:
            test_hint=torch.tensor(self.test_hintsocre[str(q_id)])
            if "bias" in entry:
                return features, ques, target, entry["bias"],q_id,test_hint,entry['type_b']
            else:
                return features, ques, target, 0,q_id,test_hint,0
